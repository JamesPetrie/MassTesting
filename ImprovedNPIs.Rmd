...
dtSweep =  data.table(expand.grid(FracTest = seq(0.5,1, length.out=200), IsoEffect =  seq(0.0,1, length.out=200)))
...
p = ggplot() + geom_line(data= dtRe[Label %in% showLabels],aes(x = FracTest, y = IsoEffect, colour = Label), linewidth = 1.2) + scale_x_continuous(limits= c(0.6,1), n.breaks = 5) + scale_y_continuous(limits= c(0.6,1), n.breaks = 5 ) + geom_hline(yintercept = ausIsoEffect, colour = "purple", linetype = "dashed", linewidth = 1.2) + ylab("Isolation Effectiveness") + xlab("Fraction Regularly Testing") + ggtitle("Testing and isolation needed \n to keep Re below 1") + guides(colour=guide_legend(title="Test Protocol"))+ coord_cartesian(expand=FALSE) 
...
dt = data.table(expand.grid(ImportRate = 10^seq(-11, -2, length.out = 200), TestPeriod  = seq(0:4), TestDelay = 1))
dt = rbindlist(llply(1:nrow(dt), function(i){
  
  importRate = dt[i, ImportRate]
...
dtAll = copy(dt)
dtAll[, Strategy := "Continuous Testing"]


dt[, Capacity := 2*1/24]
dt[ Capacity < 1/24 + 24*TestPeriod*FractionInfectedDaily, Cost := NA]

dt[,TestPeriod := as.factor(TestPeriod)]
ggplot(dt,aes(x= FractionInfectedDaily, y = Cost, colour = TestPeriod)) + geom_line() + scale_x_log10() +scale_y_log10(limits = c(0.01, 0.5), n.breaks = 8) 
#+ geom_vline(xintercept = 1e-5, linetype = "dashed") + geom_vline(xintercept = 1e-9, linetype= "dashed")#+ylim(0,0.3)


```
Todo: change from import rate to daily cases (can convert to prevalence based on infectious period)
Figure 3: Daily cost of strategy depending on disease prevalence. Cut off when prevalence exceeds testing capacity
...
dt = dtAll[TestPeriod ==2 & TestDelay == 1]
...
dt[,TestPeriod := as.factor(TestPeriod)]
ggplot(dt[ TestPeriod ==2 & TestDelay == 1],aes(x= ImportRate, y = Cost, colour =Strategy)) + geom_line() + scale_x_log10() +scale_y_log10(limits = c(0.0002, 0.5), n.breaks = 8) + geom_vline(xintercept = 1e-5, linetype = "dashed") + geom_vline(xintercept = 1e-9, linetype= "dashed")#+ylim(0,0.3)
...
Todo:
Fig 5: R0 vs mean incubation, with curves for diff testing params such that Re = 1
