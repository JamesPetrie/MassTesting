...
ggplot(dt,aes(x= ImportRate, y = Cost, colour = as.factor(TestPeriod))) + geom_line() + scale_x_log10() +ylim(0,0.5)
```
...
dt = data.table(expand.grid(ImportRate = 10^seq(-9, -2, length.out = 200), TestPeriod  = seq(0:4), TestDelay = 1))
dt$Cost = sapply(1:nrow(dt), function(i){
  computeCost(importRate = dt[i, ImportRate], R0 = R0, testPeriod = dt[i,TestPeriod], testDelay = dt[i,TestDelay], incentiveCost = 5, logisticCost = 3, poolSize = 20, pcrCost = 40 )
} )
