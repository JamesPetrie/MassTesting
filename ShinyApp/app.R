...
ui <- fluidPage("Frequent PCR Testing for Airborne Pathogens. Made by James Petrie (jimpetrie@uwaterloo.ca)",
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    tabPanel("Test Sensitivity",
                             ...
                             sliderInput("logLimitOfDetection",withMathJax(p('$$LOD_{50}  \\text{ (initial viral load (log10 copies / ml)):}$$')), min = 0.0,max = 6.0,value = 2.0, step = 0.25), 
                             sliderInput("maxSensitivity",withMathJax(p('$$S_{max}  \\text{ (peak test sensitivity:)}$$')), min = 0.0,max = 1.0,value = 0.995, step = 0.005),
                             sliderInput("testSlope",withMathJax(p('$$k  \\text{ (width of intermediate region):}$$')), min = 0.0,max = 20.0,value = 6.0, step = 2),
                    ),
...
ui2 <- navbarPage("Frequent PCR Testing for Airborne Pathogens. Made by James Petrie (jimpetrie@uwaterloo.ca)",
   ...
   tabsetPanel(
     ...
     tabPanel(
       ...
       sidebarLayout(
         sidebarPanel(
           tabsetPanel(tabPanel("Test Sensitivity",
            plotOutput("TestSensitivity", height="130px"),
           withMathJax(p('$$S_V(V) =  S_{max} \\frac{1}{1 + e^{-k(\\text{log}_{10}(V)-\\text{log}_{10}(\\text{LOD}_{50}))}}$$')),
           sliderInput("logLimitOfDetection",withMathJax(p('$$V  \\text{ (initial viral load (log10 copies / ml)):}$$')), min = 0.0,max = 6.0,value = 2.0, step = 0.25), 
           ),
           tabPanel("Infectiousness",
           plotOutput("Infectiousness", height="140px"),
           withMathJax(p('$$T_V(V) = N_c \\cdot(1 - e^{-\\theta \\frac{V^h}{V^h + K_m^h}})$$')),
...
server <- function(input, output) {
  #output$exampleImplementation <- 
   ...
   output$DailyTransmissions <- renderPlot({
     ...
     inputParams = c(logPeakLoad = 8 , maxTimeAfterPeak = 24*input$maxDaysAfterPeak, initialLogLoad = input$initialLogLoad,
                     timeToPeak =  timeToPeak, timeFromPeakTo0 = timeFromPeakTo0, fracTransmitSymptoms = 1, fracDetectSymptoms = input$fracDetectSymptoms, timeFromPeakToSymptoms = input$daysFromPeakToSymptoms*24,
                     contactsPerHour = input$contactsPerDay/24,  fracIso = 0, fracTest = 0, probTransmitMid = probTransmitMidNumeric, infectHParam = input$infectHParam,
                     precision = 0.2, maxProbTransmitPerExposure = input$maxProbTransmitPerExposure)
